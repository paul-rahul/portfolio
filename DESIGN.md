...
- **Single coordinate system:** the whole diagram is one inline SVG (`viewBox="0 0 100 100"`) — circle centers/radii, labels, the dot, and the arrow are all authored in that same 100-unit space rather than mixing HTML-positioned elements with an SVG overlay. That keeps the geometry exact (the dot sits at the real computed centroid of the three circles, not a guessed card-center coordinate) and makes the whole diagram scale as one unit at any width.
- **Circle geometry:** all three rings share one radius (r=26 of the 100-unit viewBox). Engineering and Design centers sit symmetrically upper-left/upper-right (`cx 35/65, cy 38`); Business is centered horizontally and lower (`cx 50, cy 64`). That places the true three-circle overlap centroid at `(50, 47)` — slightly above the diagram's visual center — with generous, even padding between every circle and the card edge.
- **Ring treatment:** all three circles share one thin accent-colored (`--accent`) outline (`stroke-width: 0.35` of the viewBox, reading as a delicate line rather than a bold one), no fill, no shadow. `mix-blend-mode: multiply` (light mode only, reset to `normal` in dark) darkens the overlaps the way real overlapping circles would.
- **Label treatment:** ENGINEERING / DESIGN / BUSINESS use the standard mono label system (`--font-mono`, uppercase, tracked) in primary ink (`--ink`), sized proportionally to the viewBox rather than with `clamp()` — because the whole SVG scales as one unit, the labels scale with it automatically.
- **Label placement:** Engineering and Design sit at the same relative position inside their own circles — centered horizontally on their own circle's center, nudged up toward the "middle-left"/"middle-right" region and clearly outside the triple-overlap — so the two mirror each other exactly. Business sits on its own circle's horizontal center, a few units above its circle's literal vertical center, positioned so it's just outside the Engineering/Design circles too (not merely "not on the overlap line") and stays well clear of the dot and arrow above it.
- **Center dot + arrow + annotation:** a small accent-filled dot (r=0.8, a subtle `drop-shadow` glow only) marks the actual three-way overlap centroid. A short, thin curved path (`stroke-width: 0.45`, slightly heavier than the rings) with a small SVG marker arrowhead runs from just right of the dot, curving gently down-left into it — deliberately short so it never crosses into the Business circle's label or the Design label. "This is me" sits just above and right of the arrow's start, in Caveat (script/hand font, see Typography), reading right-to-left as "This is me → arrow → dot," and stays clear of the arrow stroke, the Design label, and the Business label.
