...
- **Assets:** `src/assets/how-i-operate/how-i-operate-{desktop,tablet,mobile}.svg`. Each is a self-contained `<svg viewBox="...">` with its own `<defs>` (arrowhead `<marker>`, a small `<style>` block for shared classes) and inline text/path content for all five steps plus the `05 → 01` feedback loop.
- **Rendering:** imported via Vite's `?raw` suffix and injected with Astro's `set:html` directive (`src/pages/career/index.astro`) — not `<img>`/`<object>`/`<embed>`, which would load the SVG as an external document and not inherit the page's CSS custom properties. Every color and font in the SVGs is a `var(--token, fallback)` reference (`--accent`, `--ink`, `--ink-secondary`, `--background`, `--font-mono`, `--font-heading`) matching this project's actual token names exactly, so Day/Night both work with zero SVG-specific theme code and zero separate Day/Night SVG files.
