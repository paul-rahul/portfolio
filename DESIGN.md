...
- **Responsive:** ≥981px is the 30/70 split above. 681–980px, `.career-explorer` drops to a single column and `.firm-pane` stops being sticky (`position: static`) — firm cards, project chips and the project detail all stack full-width in document order (firm list → selected firm → role history → project list → selected project → accordion), same as desktop's single-panel-at-a-time model. No breakpoint hides or horizontally scrolls any of this content.
- **Compact mobile inline accordion (≤680px only):** each `.firm-card` becomes an accordion trigger for its own `.firm-detail` panel, which renders directly beneath that card instead of in the shared `.firm-detail-pane` further down the page. Only one employer panel is expanded at a time; tapping a different firm collapses the current one and expands the tapped one in place, tapping the expanded firm collapses it with none left open. This is DOM reparenting, not duplication: the panel node itself moves between `.firm-detail-pane` (its desktop/tablet home) and the clicked firm's `<li>` (its compact home) via `Node.appendChild`/`insertBefore` — same element, same ids, same nested project chips and accordion triggers, so there's exactly one source of truth and no duplicate `aria-controls`/IDs to keep in sync. A single `matchMedia("(max-width: 680px)")` `change` listener drives the reparenting when the breakpoint is crossed (not a per-resize-event listener), restoring every panel to its original position in `.firm-detail-pane` (by firm order) when widening past 680px, and moving the currently-selected firm's panel inline when narrowing below it. The firm trigger button carries `aria-expanded` (kept in sync with the panel's visibility at every breakpoint, not just compact) alongside the pre-existing `aria-pressed`/active-state styling, plus a chevron indicator (`.firm-card-chevron`, CSS-only rotate on `aria-expanded="true"`) that's hidden above 680px. Selecting a firm still updates `location.hash` the same way as desktop. Motion matches the existing firm-switch convention used everywhere else in the explorer: an entrance-only `motion-firm-enter` fade-up on expand, immediate hide on collapse — no separate mobile animation language, and nothing to gate behind `prefers-reduced-motion` beyond the site-wide global rule already in place.
...
- **Architecture: six immutable pre-colored SVGs, rendered as external images.** Superseding the earlier inline-SVG-with-CSS-variables approach: instead of three theme-agnostic SVGs whose colors were resolved live via `var(--token)`, there are now six complete SVG files — one per breakpoint (desktop/tablet/mobile) × theme (dark/light) — with every color baked in as a literal hex value matching this project's actual Day/Night token values (dark: `--accent #8b97ff` / `--ink #f2f2f0` / `--background #121214`; light: `--accent #4c5ff5` / `--ink #111111` / `--background #f6f6f3`). Nothing in these files references a CSS custom property. CSS's only job is choosing which one of the six is visible.
- **Assets:** `src/assets/how-i-operate/how-i-operate-{desktop,tablet,mobile}-{dark,light}.svg`. Same internal geometry as before (five steps, one arrowhead `<marker>` with `markerUnits="userSpaceOnUse"`, the `05 → 01` feedback loop) — only the fill/stroke values differ between the dark and light variant of a given breakpoint.
- **Rendering:** imported as ordinary Vite/Astro image assets (`import desktopDark from "../../assets/how-i-operate/how-i-operate-desktop-dark.svg"`, no `?raw` suffix) and rendered through a plain `<img src={desktopDark.src} width={desktopDark.width} height={desktopDark.height} />` — deliberately **not** inlined this time. Inlining would let the page's CSS reach into the SVG and repaint it, which is exactly what this architecture avoids: colors are fixed per file, so the browser must not inherit or override anything. `width`/`height` come from each file's own intrinsic size (its `viewBox`) so the browser can reserve layout space before the image loads, avoiding layout shift.
- **Breakpoint + theme switching:** six wrapper `<div class="how-i-operate-asset how-i-operate--{breakpoint}-{theme}">`, all `display: none` by default. `:root[data-theme="dark"] .how-i-operate--desktop-dark` / the light equivalent set the base (`>980px`) visible pair. The `≤980px` media block force-hides the desktop pair with `!important` (needed this time because, unlike a single breakpoint-only toggle, the dark/light pair at the same breakpoint sit at equal specificity within the same media query — `!important` is what lets the tablet rule win) and shows the theme-matched tablet pair; the `≤680px` block repeats the pattern for tablet → mobile. Exactly one of the six is visible at any width/theme combination.
- **Global `img` rule override:** the site has a global `img { width: 100%; height: 100%; object-fit: cover; }` rule (tokens.css) intended for photographic content. Left alone, it would crop these SVGs. `.how-i-operate-asset img { height: auto; object-fit: contain; }` overrides it — `.class img` (specificity 0,1,1) beats the bare `img` (0,0,1) regardless of source order — so these images always render at their true aspect ratio.
- **Non-negotiable ownership split:** the SVG files own every visual detail — icon/step/arrow/loop/label geometry *and* now color/font too. CSS owns only which of the six files is visible, wrapper width, and outer section spacing. Do not recolor, restyle, or reposition anything inside these SVGs from CSS — if the design needs to change, edit the SVG file for that breakpoint+theme pair (and its counterpart, to keep geometry identical across the pair).
- **Accessibility:** all six `<img>` elements carry `alt=""` and `aria-hidden="true"` (decorative — only one is ever visible, and screen readers should never encounter the same content six times). The one accessible representation is a `.visually-hidden` `<ol>` listing the five steps plus "Seek feedback and iterate back to understanding the system.", inside a section labelled by a `.visually-hidden` `<h2>`.
- **Motion:** unchanged — `revealOnce()` targets the section as a single unit (`asUnit: true`), reusing the shared `.motion-pre-reveal.is-revealed` rule used elsewhere for single-block reveals (Built, Internships, Contact). No animation touches anything inside the images. Reduced-motion is unaffected by the existing global rule.
