...
- "How I operate" is no longer a CSS grid — see **"How I operate" infographic** below for the canonical-SVG architecture that replaced it.
...
- **Architecture: canonical SVG, not CSS-computed layout.** After several rounds of trying to reproduce an approved reference design with responsive CSS (grid tracks, `clamp()` sizing, hand-built connector SVGs), the approach was replaced entirely: three complete, hand-authored SVG files — one per breakpoint — are the single source of truth for icon geometry, step placement, arrow geometry, the feedback loop, and the feedback label. CSS no longer computes or positions any of that; it only decides which SVG is visible.
- **Assets:** `src/assets/how-i-operate/how-i-operate-{desktop,tablet,mobile}.svg`. Each is a self-contained `<svg viewBox="...">` with its own `<defs>` (one arrowhead `<marker>`, `markerUnits="userSpaceOnUse"` so the arrowhead size never scales with a path's `stroke-width`) and inline `fill`/`stroke`/`font-family` attributes directly on each element (no shared `<style>` block) for all five steps plus the `05 → 01` feedback loop.
- **Rendering:** imported via Vite's `?raw` suffix and injected with Astro's `set:html` directive (`src/pages/career/index.astro`) — not `<img>`/`<object>`/`<embed>`, which would load the SVG as an external document and not inherit the page's CSS custom properties. Every color and font in the SVGs is a `var(--token, fallback)` reference (`--accent`, `--ink`, `--background`, `--font-mono`, `--font-heading`) matching this project's actual token names exactly, so Day/Night both work with zero SVG-specific theme code and zero separate Day/Night SVG files.
- **Breakpoint switching:** `.operate-svg--desktop` / `--tablet` / `--mobile` wrappers, toggled with plain `display: none`/`block` across the existing breakpoint contract (`>980px` / `681-980px` / `≤680px`) — see Responsiveness above. Each SVG scales via `width: 100%; height: auto` against its own `viewBox`, never `transform: scale()` or `zoom`, so there is no per-width sizing math and therefore no class of "does the CSS match the target" bug left to have.
- **Non-negotiable ownership split:** the SVG files own icon/step/arrow/loop/label geometry; CSS owns only which variant is visible, its width, and outer section spacing. Do not reintroduce per-element CSS positioning for steps, arrows, or the feedback loop — if the design needs to change, edit the SVG file for that breakpoint.
- **Accessibility:** all three SVG wrappers carry `aria-hidden="true"` (decorative from the page's semantic perspective, since only one is ever visible and screen readers shouldn't encounter the same content three times). The one accessible representation is a `.visually-hidden` `<ol>` listing the five steps plus "Seek feedback and iterate back to understanding the system." — real DOM content for assistive tech, not derived from the SVG's own internal `<title>`/`<desc>` (which remain in each file for standalone/non-inlined use, with unique ids per breakpoint since three copies now share one document).
- **Motion:** `revealOnce()` targets the section as a single unit (`asUnit: true`) — the whole graphic fades/slides up together on scroll into view, reusing the shared `.motion-pre-reveal.is-revealed` rule already used elsewhere for single-block reveals (Built, Internships, Contact). No animation touches anything inside the SVGs. Reduced-motion is unaffected by the existing global rule.
