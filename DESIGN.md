...
| **Comfortable / tablet** | 681–980px | Tablets and small laptops. Header nav has already wrapped under the brand row (see Navigation below); a narrow `(max-width: 980px) and (min-width: 681px)` band exists specifically to ease density in components that would otherwise inherit desktop spacing untouched (`/internships`' pinned timeline, metric grid). Career's employer explorer switches to the same inline-accordion interaction as mobile at this width — see **Firm/role/project explorer** below. |
...
- "How I operate" is no longer a CSS grid — see **"How I operate" infographic** below for the six-asset SVG architecture that replaced it.
...
- Desktop layout is a 30/70 split: sticky `.firm-pane` (min 260px, 30% width) + `.firm-detail-pane` (70%). Below 980px (tablet and mobile alike) it switches to the inline employer accordion — see **Firm/role/project explorer** below.
...
- Career: the explorer container (firm list + detail pane) reveals once as a whole; "How I operate" reveals as one unit (`asUnit`) — see **"How I operate" infographic** below for why it's no longer a per-step stagger. Firm/project switching keeps its own state-change animation (see below) — it never triggers or replays the scroll reveal.
...
Item-level stagger is reserved for sequences where the order itself aids scanning or explains a system: Homepage Capabilities (4 items, numbered), the two-card About editorial grid, and Career's project metric tiles. Career "How I operate" no longer staggers — see **"How I operate" infographic** below.
...
- **Responsive:** ≥981px is the 30/70 split above. ≤980px (tablet and mobile alike) switches to the inline employer accordion described below — there is no longer an intermediate "stacked but detached" tablet layout.
- **Inline employer accordion (≤980px, tablet and mobile share this interaction):** each `.firm-card` becomes an accordion trigger for its own `.firm-detail` panel, which renders directly beneath that card instead of in the shared `.firm-detail-pane` further down the page. Only one employer panel is expanded at a time; tapping a different firm collapses the current one and expands the tapped one in place, tapping the expanded firm collapses it with none left open. This is DOM reparenting, not duplication: the panel node itself moves between `.firm-detail-pane` (its desktop home) and the clicked firm's `<li>` (its compact home) via `Node.appendChild`/`insertBefore` — same element, same ids, same nested project chips and accordion triggers, so there's exactly one source of truth and no duplicate `aria-controls`/IDs to keep in sync. A single `matchMedia("(max-width: 980px)")` `change` listener drives the reparenting when the breakpoint is crossed (not a per-resize-event listener), restoring every panel to its original position in `.firm-detail-pane` (by firm order) when widening past 980px, and moving the currently-selected firm's panel inline when narrowing below it. The firm trigger button carries `aria-expanded` (kept in sync with the panel's visibility at every breakpoint, not just compact) alongside the pre-existing `aria-pressed`/active-state styling, plus a chevron indicator (`.firm-card-chevron`, CSS-only rotate on `aria-expanded="true"`) that's hidden above 980px. Selecting a firm still updates `location.hash` the same way as desktop. Motion matches the existing firm-switch convention used everywhere else in the explorer: an entrance-only `motion-firm-enter` fade-up on expand, immediate hide on collapse — no separate mobile animation language, and nothing to gate behind `prefers-reduced-motion` beyond the site-wide global rule already in place.
- **Tablet vs. mobile density:** the two share interaction but not spacing. In the 681–980px band, `.firm-card`/`.project-chip` keep their default `16px 18px` padding and `.metric-row` shows 2 columns (via the existing `(max-width: 980px) and (min-width: 681px)` band, unchanged by this refactor). At ≤680px, `.firm-card`/`.project-chip` padding drops to `14px`, accordion trigger/panel padding compresses, and `.metric-row` drops to 1 column.
...
### "How I operate" infographic (career page)

- **Architecture: six immutable pre-colored SVGs, rendered inline.** One SVG per breakpoint (desktop/tablet/mobile) × theme (dark/light), each with every color baked in as a literal hex value (dark: accent `#6F84FF` / ink `#F2F2F0` / pill fill `#111214`; light: accent `#5368F7` / ink `#121214` / pill fill `#F7F7F4`) — no CSS custom properties anywhere inside these files. Unlike the prior external-`<img>` round, these are inlined via `?raw` again; since colors are fixed per file rather than resolved from a `var()`, inlining no longer risks the page's CSS repainting them, and inlining is what lets a single visible-heading section keep everything as real DOM (see below).
- **Assets:** `src/assets/how-i-operate/how-i-operate-{desktop,tablet,mobile}-{dark,light}.svg`. Geometry per breakpoint follows the approved targets: desktop `viewBox="0 0 2048 369"`, tablet `viewBox="0 0 1462 510"` (title wraps "Align" / "the team" to two lines here), mobile `viewBox="0 0 480 650"` with exactly four downward connector arrows and the return loop on the left side (arrowhead only at 01, none at 05).
- **The "How I operate." heading is real HTML, not part of any SVG.** Earlier rounds' SVGs drew the heading as `<text>` inside the graphic; these six final files don't — the visible heading lives outside the SVGs as an ordinary `<h2>`, reusing the same `.section-heading > div > h2` pattern already used for "What I'm known for." on the homepage. There is exactly one visible heading regardless of which of the six SVGs is showing.
- **Rendering:** imported via Vite's `?raw` suffix and injected with Astro's `set:html` directive — six wrapper `<div class="how-i-operate-asset how-i-operate--{breakpoint}-{theme}" aria-hidden="true" set:html={...} />`. Each file carries unique `id`s (`how-i-operate-{breakpoint}-{theme}-title`/`-desc`, `operate-arrow-{breakpoint}-{theme}`) — required because inlining puts all six copies in one DOM at once, unlike the external-`<img>` round where identical ids across files were harmless.
- **Breakpoint + theme switching:** identical cascade to the prior round, just targeting `<svg>` instead of `<img>`: all six wrappers `display: none` by default; `:root[data-theme="dark"] .how-i-operate--desktop-dark` / the light equivalent show the base (`>980px`) pair; the `≤980px` block force-hides the desktop pair with `!important` (needed because the dark/light pair at a given breakpoint sit at equal specificity within the same media query) and shows the tablet pair; `≤680px` repeats the pattern for tablet → mobile. Exactly one of six visible at any width/theme combination.
- **Non-negotiable ownership split:** the SVG files own every visual detail — icon/step/arrow/loop/label geometry *and* color/font. CSS owns only which of the six is visible, wrapper width, and outer section spacing (`display`, `width`, `max-width`, `height: auto` on the `<svg>` — never `transform: scale`, `zoom`, `height: 100%`, or `object-fit: cover`). If the design needs to change, edit the SVG file for that breakpoint+theme pair.
- **Accessibility:** all six SVG wrappers carry `aria-hidden="true"` (only one is ever visible, and screen readers shouldn't encounter the same content six times). The one accessible representation of the process steps is a `.visually-hidden` `<ol>`; the heading itself needs no sr-only duplicate since it's already real, visible content.
- **Motion:** unchanged — `revealOnce()` targets the section as a single unit (`asUnit: true`), reusing the shared `.motion-pre-reveal.is-revealed` rule used elsewhere for single-block reveals (Built, Internships, Contact). No animation touches anything inside the SVGs. Reduced-motion is unaffected by the existing global rule.
